...
public class Crowd {
	 ...
	 *            Maximum simultaneous uploads performed by the initial source.
	 */
	public Crowd(int initialSourceUpSlotsMu, int initialSourceUpSlotsSigma) {

		//
		// Prepare the initial source (the first peer)
		//
		initialSource = new Peer(1, 0, 0, initialSourceUpSlotsMu,
				initialSourceUpSlotsSigma, 0, 0, 0);
		initialSource.setAsInitialSource();
	 ...
	 */
	public LinkedList<Peer> addPeers(int peersNum, int downSlotsMu,
			int downSlotsSigma, int upSlotsMu, int upSlotsSigma,
		...
		for (int i = (peers.size() + 1); i <= lastPeerSerial; i++) {
			Peer peer = new Peer(i, downSlotsMu, downSlotsSigma, upSlotsMu, upSlotsSigma, downPendingMax,
					knownPeersMax, requestTtl);
	...
	public void setSpecialPeers(SpecialType specialType, int specialPeersNum) {
		...
		float factorList[] = { 0.2f, 0.4f, 0.5f, 2f, 3f, 5f };
		...
		case DOWN_SLOTS:
			...
			for (float curFactor : factorList) {
				for (int i = 0; i < groupSize; i++, curSpecialIndex++) {
					Peer curPeer = peers.get(curSpecialIndex);
					curPeer.setValue(specialType,
							(int) (curFactor * normalValue));
				}
			}
			break;
		}
	...
	private void setPeerInitialKnownPeers(Peer curPeer,
			int initialKnownPeersNum, int roundToFirstUse) {
	 ...
	 */
	public int play(boolean print, int churnSeederLeavePercents,
			int pieceSlotTimeSec, int roundsPerPeerExchange,
		...
		for (round = 1;; round++) {
			...
			//need to change the order of peers
			int[] permutation = PermutationGen.generate(peers.values().size());
			List<Peer> peerC = new ArrayList<Peer>(peers.values());
			for (int i : permutation)
			{
				Peer peer = peerC.get(i);
				
				if (!peer.isActive())
					continue;

				//Reset peer download/upload bandwidth
				peer.resetBandwidths();
			...
			int uploads = 0;
			//need to change the order of peers
			permutation = PermutationGen.generate(peers.values().size());
			peerC = new ArrayList<Peer>(peers.values());
			for (int i : permutation) {
				Peer peer = peerC.get(i);
			...
			System.out
					.println(String
							.format("%,5d\t%,7d\t%8.2f\t%,5d\t%,5d\t%,5d\t%,8d\t%,7d\t%,5d\t%11.1f\t%10.1f\t%9d\t%,9d\t%,9d\t%,9d",
									round,
									round * pieceSlotTimeSec,
									downloadedPiecesAverage * 100
											/ Main.piecesNum,
									activePeers.size(),
									seeders,
									getSourcesCount(),
									...
									uploads,
									exchanges,
									getKnownPeersAverage(),
									getUpPendingAverage(),
									getPiecesMin(),
									getPeersBehind((int) (downloadedPiecesAverage - 20)),
									getPeersBehind((int) (downloadedPiecesAverage - 40)),
	...
	public void printSpecialPeersSummary() {
		...
		for (Peer curPeer : peers.values()) {
			...
			float curFactor = ((float) curPeer.getValue(specialType))
					/ normalValue;
	...
	public int performSeedersChurn() {
		...
		for (Peer curPeer : peers.values()) {
			if (curPeer.isSeeder() && curPeer.isActive()) {
				if (!curPeer.isInitialSource()
						&& shouldChurn(churnSeederLeavePercents)) {
	...
	public double getUploadsStdev() {
		float average = (peers.size() - 1) * Main.piecesNum
				/ (float) peers.size();
